<script>
  ...
  onMount(async () => {
    ...
    deleteButton.addEventListener("click", async () => {
      ...
      try {
        const response = await fetch(`http://${domain}:3000/staff/${id}`, {
          method: "DELETE",
        });
        if (response.ok) {
...
<div class="flex flex-col gap-8">
  <h1 class="text-4xl text-center py-8">Borrar Staff</h1>
  ...
  <div class="flex justify-center">
    ...
    <button
      ...
      onclick="window.location.href = '/federacion/staff'">Cancelar</button
    >
  </div>
</div>
